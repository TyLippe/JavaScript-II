...
let fullName = []; //created array for fullName
    runners.forEach(function(object){ //told it that for each object to take first and last name, creat a name object and then push that object to new array. 
...
let largeShirts = [];
    runners.filter(function(object) {
    if (object.shirt_size === 'L'){
        largeShirts.push(object);
    }
    else {
        return false;
    }
...
let ticketPriceTotal = [];
    runners.map(function(object){
    let don = object.donation
    ticketPriceTotal.push(don);
    })

let donations = ticketPriceTotal.reduce(function(acc, value){
    return acc + value;
})
...
// Problem 1 Who donated more than 100?
let oneHundred = [];
    runners.filter(function(object) {
    if (object.donation >= 100){
        oneHundred.push(object);
    }
    else {
        return false;
    }
...
// Problem 2 Sort the names out
let inOrder = [];
    runners.map(function(object){
    let order = object.first_name + ' ' +object.last_name;
    inOrder.push(order)
})
console.log(inOrder.sort()) 

// Problem 3 Amount of small shirts needed
let smallShirts = [];
    runners.filter(function(object) {
    if (object.shirt_size === 'S'){
        smallShirts.push(object);
    }
    else {
        return false;
    }
})
console.log(smallShirts.length, 'Small shirts are needed');